...
def init_middle_json():
    return {
        "pdf_info": [],
        "_backend": "vlm_native_correction",
        "_version_name": __version__,
        "_correction_metrics": [],
    }


def append_page_correction_to_middle_json(
    middle_json,
    ...
    for offset, (page_blocks, metrics, image_dict) in enumerate(
        zip(corrected_blocks_list, metrics_list, images_list)
    ):
        ...
        middle_json["_correction_metrics"].append(metrics.to_dict())
